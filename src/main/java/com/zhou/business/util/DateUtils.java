...
public class DateUtils {
    ...
    public final static String shortFormat1 = "yyMMdd";
    ...
    public static String getShortDay(Date date){
        try {
            DateFormat dateFormat = new SimpleDateFormat(shortFormat1);
            return dateFormat.format(date);
        } catch (Exception ex) {
            log.error("解析日期错误,str=" + date);
            return null;
        }
    }
    ...
    /**
     * 计算2个时间的差异分钟数，如果leftSecondsToMinutes为ture，且日期相差的秒数数不能被60整除(表示2个日期差，有X分Y秒)
     * ， 则额外增加1分返回
     *
     * @param one
     * @param two
     ...
     * @return 计算后的日期
     */
    public static Date addDate(Date date, int calendarType, int var){
        Calendar c = Calendar.getInstance();
        c.setTime(date);
        if(calendarType == Calendar.MINUTE) {
            c.add(Calendar.MINUTE, var); //加减分钟数
        }
        if(calendarType == Calendar.HOUR) {
            c.add(Calendar.HOUR, var); //加减小时
        }
        if(calendarType == Calendar.SECOND){
            c.add(Calendar.SECOND,var);
        }
        return c.getTime();
    }
     ...
     */
    public static String getFormatDate(Date date,String daFormat) {
